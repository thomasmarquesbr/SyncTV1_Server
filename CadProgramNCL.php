...
function valida(form) {
	...
	if (form.dispon.value == "") 
	{
		alert("O período de disponibilidade não foi preenchido!");
		form.dispon.focus();
		return false;
	}
	if(!this.arquivo.value){
		alert("Nenhum arquivo NCL foi selecionado");
		form.arquivo.focus();
		return false;
	}else{
		extensao = (this.arquivo.value.substring(this.arquivo.value.lastIndexOf("."))).toLowerCase();
		if(extensao != ".ncl"){
			alert("O arquivo não é um documento NCL!");
			return false;
		}
	}
}
...
<?php 	
	...
	{//conectado com o servidor
		...
		{//conectado com o banco de dados
			...
			{
				echo "
					<form name=\"form1\" action=\"CadProgramNCL.php\" method=\"get\" >
					<div class=\"table2CSS\">
					<table>
					  <tr>
				...
				echo "
						<form name=\"form\" action=\"getCadProgNCL.php\" method=\"post\" onsubmit=\"return valida(this);\" enctype=\"multipart/form-data\">
						  ...
						  <tr>
						  	<td>Documento NCL principal</td>
							<td><input type=\"file\" id=\"arquivo\" name=\"arquivo\" disabled=\"true\"/></td>
						  </tr>
						  <tr>
						  	<td>Fuso:</td>
							<td>
